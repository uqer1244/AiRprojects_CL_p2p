...
class PeerProtocol:
    ...
    def datagram_received(self, data: bytes, addr: tuple):
        print(f"[{self.node_id}] 📥 P2P UDP 메시지 수신 from {addr}: {data.decode()}")
...
async def run_client(node_id: str, p2p_port_req: int, cmd_port_req: int):
    ...
    try:
        p2p_transport, _ = await loop.create_datagram_endpoint(
            lambda: PeerProtocol(node_id), local_addr=('0.0.0.0', p2p_port_req)
        )
        actual_p2p_port = p2p_transport.get_extra_info('sockname')[1]

        cmd_transport, _ = await loop.create_datagram_endpoint(
            lambda: CommandProtocol(p2p_transport, my_p2p_peers, node_id, gps_queue, first_gps_event),
        ...
        try:
            ...
            def cleanup_port_file():
                try:
                    ...
                    if node_id in final_port_data:
                        del final_port_data[node_id]
                    with open("p2p_ports.json", "w") as f:
    ...
    while True:
        try:
            async with websockets.connect(server_uri) as websocket:
                ...
                async def handle_server_messages():
                    async for message in websocket:
                        ...
                        if msg_type == "group_update":
                            ...
                            print(f"    -> ✅ [{node_id}] P2P 연락처 추가 완료! 현재 P2P 친구: {list(my_p2p_peers.keys())}")
                            res_msg = {"type": "p2p_response", "target_id": sender_id, "sender_id": node_id,
                                       "port": actual_p2p_port}
                            await websocket.send(json.dumps(res_msg))
                            if p2p_transport:
                                p2p_transport.sendto(f"Punch from {node_id}".encode(), (sender_ip, sender_port))
                ...
                await asyncio.gather(handle_server_messages(), send_location())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="P2P Test Client - Waits for GPS")
    ...
    if not args.id:
        adjectives = ["Brave", "Clever", "Fast", "Silent", "Wise", "Happy", "Lucky", "Shiny"]
        nouns = ["Lion", "Tiger", "Fox", "Eagle", "Wolf", "Panda", "Dragon", "Unicorn"]
